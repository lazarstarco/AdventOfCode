...
function getDigit(str) {
  res =
    /(one)|(two)|(three)|(four)|(five)|(six)|(seven)|(eight)|(nine)|(1)|(2)|(3)|(4)|(5)|(6)|(7)|(8)|(9)|(eno)|(owt)|(eerht)|(ruof)|(evif)|(xis)|(neves)|(thgie)|(enin)/i.exec(
      str
    );
  return res[0];
}
...
function getFirst2(str) {
  extracted = getDigit(str);
  digit = mapDigit(extracted);
  return digit;
}

function getLast2(str) {
...
sum1 = sum2 = 0;
strgs.forEach((str) => {
  sum1 += getFirst1(str) * 10 + getLast1(str);
  sum2 += getFirst2(str) * 10 + getLast2(str);
});

console.log({ 1: sum1, 2: sum2 });
